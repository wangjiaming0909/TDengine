...
static int32_t buildDataBlockSlots(SPhysiPlanContext* pCxt, SNodeList* pList, SDataBlockDescNode* pDataBlockDesc,
  ...
  FOREACH(pNode, pList) {
    ...
    if (TSDB_CODE_SUCCESS == code) {
      ...
      qDebug("wjm pust slot key: %s len: %d datablockid: %d", name, len, pDataBlockDesc->dataBlockId);
    }
    taosMemoryFree(name);
    if (TSDB_CODE_SUCCESS == code) {
...
static int32_t addDataBlockSlotsImpl(SPhysiPlanContext* pCxt, SNodeList* pList, SDataBlockDescNode* pDataBlockDesc,
  ...
  FOREACH(pNode, pList) {
    ...
    qDebug("wjm node: %s add db slot impl slot id: %d dbid: %d", name, slotId, pDataBlockDesc->dataBlockId);
    taosMemoryFree(name);
    if (TSDB_CODE_SUCCESS == code) {
      SNode* pTarget = NULL;
...
static EDealRes doSetSlotId(SNode* pNode, void* pContext) {
  if (QUERY_NODE_COLUMN == nodeType(pNode) && 0 != strcmp(((SColumnNode*)pNode)->colName, "*")) {
    ...
    qDebug("wjm node: %s set slot id: %d dbid: %d ", name, ((SColumnNode*)pNode)->slotId, ((SColumnNode*)pNode)->dataBlockId);
    taosMemoryFree(name);
...
static int32_t setNodeSlotId(SPhysiPlanContext* pCxt, int16_t leftDataBlockId, int16_t rightDataBlockId, SNode* pNode,
  ...
  qDebug("wjm start to set node slot id, left dbid: %d right dbId: %d", leftDataBlockId, rightDataBlockId);
  SSetSlotIdCxt cxt = {
...
static SPhysiNode* makePhysiNode(SPhysiPlanContext* pCxt, SLogicNode* pLogicNode, ENodeType type) {
  ...
  qDebug("wjm create physic node: %d", type);
  if (NULL == pPhysiNode) {
    terrno = code;
    return NULL;
  }

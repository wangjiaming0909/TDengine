 ...
 */
static bool filterWindowWithLimit(SIntervalAggOperatorInfo* pOperatorInfo, TSKEY key, bool isCalculatedWin) {
  ...
  if (taosBQSize(pOperatorInfo->pBQ) == taosBQMaxSize(pOperatorInfo->pBQ) + 1) {
    ...
    shouldFilter = tsKeyCompFn(top->data, &key, pOperatorInfo);
  }
  if (shouldFilter) {
    return true;
  } else if (isCalculatedWin) {
    return false;
  }
...
static void hashIntervalAgg(SOperatorInfo* pOperatorInfo, SResultRowInfo* pResultRowInfo, SSDataBlock* pBlock,
  ...
  if (filterWindowWithLimit(pInfo, win.skey, isCalculatedWin(pInfo, &win, tableGroupId))) return;
  ...
  while (1) {
    ...
    if (startPos < 0 || filterWindowWithLimit(pInfo, nextWin.skey, isCalculatedWin(pInfo, &nextWin, tableGroupId))) {
      break;
    }
